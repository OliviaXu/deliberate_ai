...
export function main() {
  ...
  const userDataDir = path.resolve(projectRoot, process.env.CHATGPT_USER_DATA_DIR || '.pw-profiles/chatgpt');
  ...
  const chromeBinary =
    process.env.CHATGPT_CHROME_BINARY ||
  ...
  console.log(headless ? 'Reused the existing ChatGPT session without opening a window.' : 'Sign into ChatGPT in that Chrome window, then keep it open while tests run.');
}

...
const EXTENSION_NAME = process.env.GEMINI_EXTENSION_NAME || 'Deliberate AI';
...
async function main() {
  ...
  if (!browser) {
    console.error(`Could not connect to ${cdpUrl}. Start Chrome with "npm run gemini:open" first.`);
    process.exit(1);
  }

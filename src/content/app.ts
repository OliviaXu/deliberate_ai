...
export async function startContentApp({ now }: ContentAppDependencies): Promise<void> {
  ...
  async function handleIntercept(intent: InterceptedSubmitIntent): Promise<void> {
    ...
    modalOpen = true;
    setDomState(interceptionCount, true, busyDropCount);
  ...
  async function resolveRecordHasCompletedReflection(
    learningCycleRecord: ReflectionEligibleLearningCycleRecord
  ): Promise<boolean> {
...
function setDomState(count: number, isModalOpen: boolean, busyDropCount: number): void {
